...
import { ADMIN_POSTS, ADMIN_USERS, ADMIN_STATS, type PostStatus, type AdminPost } from '@/lib/adminData'
...
export default function AdminPage() {
  ...
  const handleApprove = (id: string) =>
    setPosts((p) => p.map((post) => post.id === id ? { ...post, status: 'approved' as PostStatus } : post))

  const handleReject = (id: string) =>
    setPosts((p) => p.map((post) => post.id === id ? { ...post, status: 'rejected' as PostStatus } : post))
      ...
      <main className="ml-[240px] flex-1 p-8">
        ...
        {tab === 'overview' && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-slate-400 text-sm mb-8">Chào mừng trở lại, {user.name.split(' ').pop()}!</p>
